from collections import Counter
from itertools import chain
import os
from sympy import true
import torch
import pickle

from utils.utils import pad_sents, pad_sents_char


class Vocab:
    """
    Vocab encapsulating source and target language
    """
    def __init__(self, src_vocab, tgt_vocab):
        """init vocab

        Args:
            src_vocab (VocabEntry): VocabEntry for source language
            tgt_vocab (VocabEntry): VocabEntry for target language
        """
        self.src = src_vocab
        self.tgt = tgt_vocab

    
    def build(src_sents, tgt_sents, vocab_size, freq_cutoff):
        """Build Vocabulary
        """
        assert len(src_sents) == len(tgt_sents)
        src = VocabEntry.from_corpus(src_sents, vocab_size, freq_cutoff, collect_chars=True)
        tgt = VocabEntry.from_corpus(tgt_sents, vocab_size, freq_cutoff, collect_chars=False)

        return Vocab(src, tgt)
    
    def save(self, file_path):
        """Save Vocab to file as JSON dump."""

        # ensure the directory have existed
        dir_path = os.path.dirname(file_path)
        if not os.path.exists(dir_path):
            os.makedirs(dir_path, exist_ok=true)

        with open(file_path, "wb") as file:
            save = dict(src_word2id=self.src.word2id, 
                        tgt_word2id=self.tgt.word2id,
                        src_char2id=self.src.char2id,
                        tgt_char2id=self.tgt.char2id)
            pickle.dump(save, file)
            print("Vacabulary have saved!")

    @staticmethod
    def load(file_path):
        """Load vocabulary from JSON dump."""
        entry = pickle.load(open(file_path, "rb"))
        src_word2id = entry["src_word2id"]
        tgt_word2id = entry["tgt_word2id"]
        src_char2id = entry.get("src_char2id", None)
        tgt_char2id = entry.get("tgt_char2id", None)

        src_vocab = VocabEntry(word2id=src_word2id, char2id=src_char2id)
        tgt_vocab = VocabEntry(word2id=tgt_word2id, char2id=tgt_char2id)
        return Vocab(src_vocab, tgt_vocab)

    def __repr__(self):
        """Representation of Vocab to be used when printing the object."""
        return "Vocab(source %d words, target %d words)" % (len(self.src), len(self.tgt))
    


class VocabEntry:
    def __init__(self, word2id=None, char2id=None):
        """Init a vocabEntry instance"""

        ### word-level representation ###
        if word2id:
            self.word2id = word2id
        
        else:
            self.word2id = dict()        # word to index, we use dictionary
            self.word2id["<pad>"] = 0    # Pad Token
            self.word2id["<s>"] = 1      # Start Token
            self.word2id["</s>"] = 2     # End Token
            self.word2id["<unk>"] = 3    # Unknown Token

        # TODO: index to word, we can use a list, but here we follow the dict
        # self.id2word = [k for k, v in self.word2id.items()]
        self.id2word = {v: k for k, v in self.word2id.items()}
        self.word_unk = self.word2id["<unk>"]
        self.word_pad = self.word2id["<pad>"]

        ### Character-level representation (适配中文，不硬编码英文字母) ###
        if char2id:
            self.char2id = char2id
        else:
            self.char2id = dict()       # Dictionary mapping characters to indices.
            self.char2id["<p>"] = 0     # Pad token
            self.char2id["{"] = 1       # Start-of-word token
            self.char2id["}"] = 2       # End-of-word token
            self.char2id["<u>"] = 3     # Unknown token

        # 字符Level的库 需要对英文字母的硬编码
        # char_list = list(
        #     """ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789,;.!?:'\"/\\|_@#$%^&*~`+-=<>()[]""")
        # for c in char_list:
        #     self.char2id[c] = len(self.char2id)

        self.id2char = {v: k for k, v in self.char2id.items()}
        self.char_pad = self.char2id["<p>"]
        self.char_unk = self.char2id["<u>"]
        self.start_of_word = self.char2id["{"]
        self.end_of_word = self.char2id["}"]
        assert self.start_of_word + 1 == self.end_of_word
        
        # Word frequency information for frequency penalty
        self.word_freq = {}  # word_id -> frequency
        self.normalized_freq = {}  # word_id -> normalized frequency (0.0-1.0)

    @staticmethod
    def from_corpus(corpus, size, freq_cutoff=2, collect_chars=False):
        """Given a corpus construct a VocabEntry.

        Args:
            corpus (List[str]): Corpus of text produced by read_corpus function.
            size (int): Number of words in vocabulary.
            freq_cutoff (int, optional): If word occurs n < freq_cutoff times, drop the
                word. Defaults to 2.
            collect_chars | bool : 是否收集字符（中文需设为True）

        Returns:
            vocab_entry (VocabEntry): VocabEntry instance produced from provided corpus.
        """
        ### 1. 单词级词汇表
        vocab_entry = VocabEntry()
        word_freq = Counter(chain(*corpus))
        valid_words = [w for w, v in word_freq.items() if v >= freq_cutoff and w.strip() != ""]  # 只保留非空格字符
        print("number of word types: {}, number of word types w/ frequency >= {}: {}"
              .format(len(word_freq), freq_cutoff, len(valid_words)))
        top_k_words = sorted(valid_words, key=lambda w: word_freq[w], reverse=True)[:size]

        for word in top_k_words:
            vocab_entry.add(word)
        
        # Save word frequencies for frequency penalty
        # Calculate frequencies for all words in vocabulary
        max_freq = max(word_freq.values()) if word_freq else 1.0
        vocab_entry.word_freq = {}
        vocab_entry.normalized_freq = {}
        
        for word in vocab_entry.word2id:
            if word in word_freq:
                word_id = vocab_entry.word2id[word]
                freq = word_freq[word]
                vocab_entry.word_freq[word_id] = freq
                vocab_entry.normalized_freq[word_id] = freq / max_freq
            else:
                # For special tokens (<pad>, <s>, </s>, <unk>), set frequency to 0
                word_id = vocab_entry.word2id[word]
                vocab_entry.word_freq[word_id] = 0
                vocab_entry.normalized_freq[word_id] = 0.0

        ### 2. 字符级词汇表
        if collect_chars:
            # 收集语料中出现的所有字符
            char_freq = Counter(chain(*[list(word) for word in chain(*corpus)]))
            sorted_chars = sorted(char_freq.keys(), key=lambda c: char_freq[c], reverse=True)
            # 给每个字符分配ID（从4开始，0-3已被基础符号占用）
            current_char_id = len(vocab_entry.char2id)
            for char in sorted_chars:
                if char not in vocab_entry.char2id and char != " ":
                    vocab_entry.char2id[char] = current_char_id
                    current_char_id += 1
            # 更新字符ID到字符的映射
            vocab_entry.id2char = {v: k for k, v in vocab_entry.char2id.items()}
            print(f"char_vocabulary size: {len(vocab_entry.char2id)} (including: {len(sorted_chars)}")
        else:
            char_list = list(
                """ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789,;.!?:'\"/\\|_@#$%^&*~`+-=<>()[]""")
            for c in char_list:
                vocab_entry.char2id[c] = len(vocab_entry.char2id)
            vocab_entry.id2char = {v: k for k, v in vocab_entry.char2id.items()}
            print(f"char_vocabulary size: {len(vocab_entry.char2id)} (including: {len(char_list)}")
        return vocab_entry
    
    def add(self, word):
        """add word to VocabEntry. Return the assigned index"""
        if word not in self.word2id:
            wid = self.word2id[word] = len(self.word2id)
            self.id2word[wid] = word
            return wid
        else:
            return self.word2id[word]

    def __len__(self):
        """Compute number of words in VocabEntry."""
        return len(self.word2id)
    
    def words2charindices(self, sents):
        """Convert a list of sentences of words into a list of list of list of character
        indices.

        Args:
            sents (List[List[str]]): Sentence(s) in words.

        Returns:
            word_ids (List[List[List[int]]]): Sentence(s) in indices.
        """
        return [[[self.char2id.get(c, self.char_unk) for c in ("{" + w + "}")]
                                                     for w in s] for s in sents]

    
    def to_input_tensor_char(self, sents, device):
        """Convert a list of sentences (words) into tensor with necessary padding.
        All words are padded to max_word_length of all words in the batch.
        All sentences are padded to max_sentence_length of all sentences in the batch.

        Args:
            sents (List[List[str]]): List of sentences in words.
            device (torch.device): Device on which to load the tensor, i.e. CPU or GPU.

        Returns:
            sents_var (Tensor): Tensor of (max_sent_length, batch_size, max_word_length).
        """
        char_ids = self.words2charindices(sents)
        sents_padded = pad_sents_char(char_ids, self.char_pad)
        sents_padded = torch.tensor(sents_padded, dtype=torch.long, device=device)

        return sents_padded.permute(1, 0, 2).contiguous()
    
    def to_input_tensor(self, sents, device):
         """Convert list of sentences (words) into tensor with necessary padding for
        shorter sentences.

        Args:
            sents (List[List[str]]): List of sentences in words.
            device (torch.device): Device on which to load the tensor, i.e. CPU or GPU.

        Returns:
            sents_var (Tensor): Tensor of (max_sentence_length, batch_size).
        """
         word_ids = self.words2indices(sents)
         sents_padded = pad_sents(word_ids, self.word2id["<pad>"])
         sents_padded = torch.tensor(sents_padded, dtype=torch.long, device=device)
         return torch.transpose(sents_padded, 0, 1).contiguous()
    

    def __getitem__(self, word):
        """Retrieve word's index. Return the index for the unk token if the word is out of
        vocabulary.
        """
        return self.word2id.get(word, self.word_unk)

    def __contains__(self, word):
        """Check if a word is captured by VocabEntry."""
        return word in self.word2id

    def __setitem__(self, key, value):
        """Raise error, if one tries to edit the VocabEntry."""
        raise ValueError("Vocabulary is read-only")


    def __repr__(self):
        """Representation of VocabEntry to be used when printing the object."""
        return "Vocabulary[size=%d]" % len(self)

    def id2word(self, wid):
        """Return mapping of index to word."""
        return self.id2word[wid]
    

    def words2indices(self, sents):
        """Convert list of words or list of sentences of words into list or list of list
        of indices.

        Args:
            sents (List[str] or List[List[str]]): Sentence(s) in words.

        Returns:
            word_ids (List[int] or List[List[int]]): Sentence(s) in indices.
        """
        if type(sents[0]) == list:
            return [[self[w] for w in s] for s in sents]
        else:
            return [self[w] for w in sents]

    def indices2words(self, word_ids):
        """Convert a list of indices into a list words.

        Args:
            word_ids (List[int]): List of word ids.

        Returns:
            sents (List[str]): List of words.
        """
        return [self.id2word[w_id] for w_id in word_ids]
    

